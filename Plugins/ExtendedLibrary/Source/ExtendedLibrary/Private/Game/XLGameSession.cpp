...
AXLGameSession::AXLGameSession(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		OnCreateSessionCompleteDelegate = FOnCreateSessionCompleteDelegate::CreateUObject(this, &AXLGameSession::OnCreateSessionComplete);
		OnDestroySessionCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(this, &AXLGameSession::OnDestroySessionComplete);

		OnFindSessionsCompleteDelegate = FOnFindSessionsCompleteDelegate::CreateUObject(this, &AXLGameSession::OnFindSessionsComplete);
		OnJoinSessionCompleteDelegate = FOnJoinSessionCompleteDelegate::CreateUObject(this, &AXLGameSession::OnJoinSessionComplete);

		OnStartSessionCompleteDelegate = FOnStartSessionCompleteDelegate::CreateUObject(this, &AXLGameSession::OnStartOnlineGameComplete);
	}
}
...
*/
void AXLGameSession::OnStartOnlineGameComplete(FName SessionName, bool bWasSuccessful)
{
...
void AXLGameSession::HandleMatchHasStarted()
{
...
*
*/
void AXLGameSession::HandleMatchHasEnded()
{
	// start online game locally and wait for completion
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
...
bool AXLGameSession::IsBusy() const
{
...
EOnlineAsyncTaskState::Type AXLGameSession::GetSearchResultStatus(int32& SearchResultIdx, int32& NumSearchResults)
{
	SearchResultIdx = 0;
	NumSearchResults = 0;
...
*/
const TArray<FOnlineSessionSearchResult> & AXLGameSession::GetSearchResults() const
{
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void AXLGameSession::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void AXLGameSession::OnDestroySessionComplete(FName SessionName, bool bWasSuccessful)
{
...
bool AXLGameSession::HostSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, const FString& GameType, const FString& MapName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers)
{
	...
	if (OnlineSub)
	{
		...
		{
			HostSettings = MakeShareable(new FXLOnlineSessionSettings(bIsLAN, bIsPresence, MaxNumPlayers));
...
void AXLGameSession::OnFindSessionsComplete(bool bWasSuccessful)
{
...
void AXLGameSession::ResetBestSessionVars()
{
	CurrentSessionParams.BestSessionIdx = -1;
}

void AXLGameSession::ChooseBestSession()
{
...
void AXLGameSession::StartMatchmaking()
{
	ResetBestSessionVars();
	ContinueMatchmaking();
}

void AXLGameSession::ContinueMatchmaking()
{
	ChooseBestSession();
...
void AXLGameSession::OnNoMatchesAvailable()
{
	UE_LOG(LogOnlineGame, Verbose, TEXT("Matchmaking complete, no sessions available."));
	SearchSettings = NULL;
}

void AXLGameSession::FindSessions(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, bool bIsLAN, bool bIsPresence)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid() && CurrentSessionParams.UserId.IsValid())
		{
			SearchSettings = MakeShareable(new FXLOnlineSearchSettings(bIsLAN, bIsPresence));
...
bool AXLGameSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, int32 SessionIndexInSearchResults)
{
	bool bResult = false;
...
bool AXLGameSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, const FOnlineSessionSearchResult& SearchResult)
{
	bool bResult = false;
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void AXLGameSession::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
...
bool AXLGameSession::TravelToSession(int32 ControllerId, FName SessionName)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{

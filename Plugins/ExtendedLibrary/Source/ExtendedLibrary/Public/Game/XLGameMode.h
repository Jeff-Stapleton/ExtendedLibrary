...
UCLASS(config = Game)
class AXLGameMode : public AGameMode
{
	...
	UFUNCTION(exec)
	void SetAllowBots(bool bInAllowBots, int32 InMaxBots = 0);
	...
	virtual bool CanDealDamage(class AXLPlayerState* DamageInstigator, class AXLPlayerState* DamagedPlayer) const;
	...
	/** spawns default bot */
	class AXLAI* SpawnBot(FVector SpawnLocation, FRotator SpawnRotation);
...
protected:
	...
	//UPROPERTY()
	//TArray<AXLAIController*> BotControllers;
	...
	virtual bool IsWinner(class AXLPlayerState* PlayerState) const;
